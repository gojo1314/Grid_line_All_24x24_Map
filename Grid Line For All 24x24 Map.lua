function widget:GetInfo()
    return {
        name      = "Grid Line For All 24x24 Map",
        desc      = "Draws a grid for 12 players in NuttyB Raptor, line by line (works on all maps)",
        author    = "Gojo1314",
        date      = "2025",
        layer     = 1,
        enabled   = true
...
local function CollectGridLines()
    ...
    -- Horizontal lines (top and bottom sections)
    ...
    -- Vertical lines (left and right sections)
...
function widget:Initialize()
    Spring.Echo("[Auto Draw Grid] Running on map:", Game.mapName or "unknown")
...
local drawInterval = 0.16 -- seconds between lines (~10 frames at 60fps)
    ...
    if drawingGrid and currentLine <= #gridLines then
        ...
        if elapsed >= drawInterval then
            ...
            if currentLine > #gridLines then
                drawingGrid = false
                Spring.Echo("[Auto Draw Grid] Finished drawing all grid lines.")
            end
        end
    end
end
